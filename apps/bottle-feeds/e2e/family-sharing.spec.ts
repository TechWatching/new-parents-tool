...
test.describe('Family sharing through a browser-only backend contract fake', () => {
  ...
  test('a previously connected invitee can sign in with Google and join without uploading old history', async ({ browser }) => {
    service.seedFamily('father')
    const father = await parent(browser, 'father')
    await signIn(father.page)
    await expect(father.page.getByRole('button', { name: 'Sync now' })).toBeVisible()
    await addFeed(father.page, 'Previous family feed')
    await expect.poll(() => service.records.size).toBe(1)
    await signOut(father.page)
    ...
    await father.page.goto(invitation)
    await expect(father.page.getByText('You have an invitation. Sign in to join the shared history.')).toBeVisible()
    await signIn(father.page, 'Google')
    await expect(father.page.getByRole('button', { name: 'Accept invitation', exact: true })).toBeVisible()
    await expect(father.page.getByText('Sharing is paused.')).toHaveCount(0)
    await father.page.getByRole('button', { name: 'Accept invitation', exact: true }).click()
    await expect.poll(() => service.family?.members.length).toBe(2)
    await father.page.reload()
    await expect(father.page.getByRole('button', { name: 'Sync now', exact: true })).toBeVisible()
    await expect(father.page.getByText('Sharing is paused.')).toHaveCount(0)
    expect((await readStore(father.page))['guest:data']).toMatchObject({ feeds: [{ comment: 'Previous family feed' }] })
    expect(await readFamilyHistory(father.page)).toHaveLength(2)
  ...
  test('an uploaded guest feed stays dismissed through refresh and reload, but a new guest feed prompts', async ({ browser }) => {
    const mother = await parent(browser, 'mother')
    await addFeed(mother.page, 'Already uploaded guest bottle')
    await signIn(mother.page)
    await mother.page.getByRole('button', { name: 'Create a family', exact: true }).click()
    ...
    await expect.poll(() => service.records.size).toBe(1)
    await expect(mergeButton).toHaveCount(0)
    await sync(mother.page)
    await expect(mergeButton).toHaveCount(0)
    await mother.page.reload()
    await expect(mother.page.locator('.measure-tree-entry').getByText('Already uploaded guest bottle')).toBeVisible()
    await expect(mergeButton).toHaveCount(0)
    expect(JSON.stringify((await readStore(mother.page))['guest:data'])).toContain('Already uploaded guest bottle')

    await signOut(mother.page)
    await addFeed(mother.page, 'New unmerged guest bottle')
    await signIn(mother.page)
    await expect(mergeButton).toBeVisible()
    await mergeButton.click()
    await sync(mother.page)
    ...
    await mother.page.reload()
    await expect(mergeButton).toHaveCount(0)
    await expect(mother.page.locator('.measure-tree-entry').getByText('New unmerged guest bottle')).toBeVisible()
  })
  ...
  test('deleted family records stay device-only across sign-out until upload consent', async ({ browser }, testInfo) => {
    const mother = await parent(browser, 'mother')
    await signIn(mother.page)
    await mother.page.getByRole('button', { name: 'Create a family', exact: true }).click()
    await addFeed(mother.page, 'Retained family bottle')
    await mother.page.locator('.weight-card input[type=number]').fill('4.5')
    await mother.page.locator('.weight-card button[type=submit]').click()
    await expect.poll(() => service.records.size).toBe(2)
    await mother.page.getByRole('button', { name: 'Delete shared family', exact: true }).click()
    await mother.page.getByRole('button', { name: 'Confirm', exact: true }).click()
    await expect(mother.page.locator('.measure-tree-entry').getByText('Retained family bottle')).toBeVisible()
    await expect.poll(async () => JSON.stringify((await readStore(mother.page))['guest:data'])).toContain('Retained family bottle')
    await mother.page.reload()
    await expect(mother.page.locator('.measure-tree-entry').getByText('Retained family bottle')).toBeVisible()
    await signOut(mother.page)
    await expect(mother.page.locator('.measure-tree-entry').getByText('Retained family bottle')).toBeVisible()
    await mother.page.reload()
    await signIn(mother.page)
    await expect(mother.page.getByRole('button', { name: 'Create a family', exact: true })).toBeVisible()
    await mother.page.getByRole('button', { name: 'Create a family', exact: true }).click()
    await expect(mother.page.getByRole('button', { name: 'Upload and merge', exact: true })).toBeVisible()
    await sync(mother.page)
    expect(service.records.size).toBe(0)
    const oldHistory = await readFamilyHistory(mother.page)
    expect(oldHistory).toHaveLength(2)
    expect(oldHistory[0]?.namespace).not.toBe(oldHistory[1]?.namespace)
    await testInfo.attach('retained-family-upload-consent', { body: await mother.page.screenshot(), contentType: 'image/png' })
    await mother.page.getByRole('button', { name: 'Upload and merge', exact: true }).click()
    await expect.poll(() => service.records.size).toBe(2)
    await expect(mother.page.locator('.measure-tree-entry').getByText('Retained family bottle')).toBeVisible()
    await mother.page.getByRole('tab', { name: 'Weights', exact: true }).click()
    await expect(mother.page.locator('.measure-tree-entry').getByText('4.5 kg')).toBeVisible()
